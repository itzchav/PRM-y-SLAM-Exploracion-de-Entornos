...
def prm(self, imagen, imagen_reducida, im_bin,  imagen_color, numero_de_muestras, distancia_max, punto_inicio, punto_destino):
    path = None
    ...
    ruta_completa = os.path.join(self.carpeta, 'inicio_fin.jpg')
    ...
    while path is None:
        ...
        ruta_completa = os.path.join(self.carpeta, nombre_imagen_con_extension)
        ...
        try:
            if nx.has_path(grafo, tuple(punto_inicio), tuple(punto_destino)):
                ...
                ruta_completa = os.path.join(self.carpeta, "imagen_ruta.png")
...
class Nodo(object):
    def __init__(self):
        ...
        self.carpeta = os.path.join(carpeta_pruebas, 'Prueba_plano_dinamico')
        ...
        output_odom = os.path.join(self.carpeta, "prueba_odom")
        output_punto_h = os.path.join(self.carpeta, "prueba_punto_h")

        # Comando para grabar el tópico /odom
        cmd_odom = ["rosbag", "record", "-O", output_odom, "/odom"]

        # Comando para grabar el tópico /punto_h
        cmd_punto_h = ["rosbag", "record", "-O", output_punto_h,  "/punto_h"]
        self.proc1=subprocess.Popen(cmd_odom)
        #p1.wait()
        self.proc2=subprocess.Popen(cmd_punto_h)
        #p2.wait()
    ...
    def start(self):
        ...
        # Define la ruta a la carpeta "pruebas"
        #carpeta_pruebas = os.path.join(carpeta_raiz, 'Pruebas_explore')
        #self.carpeta = os.path.join(carpeta_pruebas, 'Prueba_plano_p1_10')
        ...
        while (not rospy.is_shutdown() and self.numero_de_imagen <= self.limite):# or self.termina==True:
            ...
            if self.x is not None and self.punto_desado_xh is not None:
                ...
                if distancia_plo < 0.15:
                 ...
                 #Obtener imagen del mapa
                    ...
                    ruta_completa = os.path.join(self.carpeta, self.map_file)
                    self.numero_de_imagen = self.numero_de_imagen + 1
                    print("submeta")
                    print(self.numero_de_imagen)
                    ...
                    ruta_completa = os.path.join(self.carpeta, "inicio.jpg")
                    ...
                    self.path=prm(self, img,imagen_reducida, img_bin, imagen_color, numero_de_muestras, distancia_k_vecinos, start, goal)
