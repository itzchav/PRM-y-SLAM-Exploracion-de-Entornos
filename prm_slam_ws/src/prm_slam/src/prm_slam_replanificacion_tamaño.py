...
def prm(self, image, im_bin, num_muestras, distancia_max, punto_inicio, punto_destino):
    path = None
    print("generanding")
    ...
    # Verificar y ajustar el punto de destino
    ...
    while path is None:
        ...
        #cv2.imshow('inicio_fin',imagen_color)
        #cv2.waitKey(0)
        #cv2.destroyAllWindows()
        ...
        image_with_path = imagen_color.copy()  # Copia la imagen original.
            ...
            if self.x is not None and self.Xhd is not None:
                ...
                # Distancia de tolerancia al punto mas cercano a la meta 
                 ...
                 #Obtener imagen del mapa
                    ...
                    sp,j=metros_a_pixeles(self,self.x,self.y)
                    ...
                    if verificar_region_blanca(img,xd,yd):
                        ...
                        nombre_imagen = "imagen_origen" + str(self.num_img)
                        nombre_imagen_con_extension = nombre_imagen + ".png"
                        
                        cv2.imwrite(nombre_imagen_con_extension, imagen_origen)

                        self.path=prm(self, img, img_bin, num_samples, k_neighbors, start, goal)
                        self.path_metros= [pixeles_a_metros(self, px, py) for py, px in self.path]
                        print(self.path)
                        bandera=0
                        
                    else:
                        xs,ys=metros_a_pixeles(self, self.Xhd, self.Yhd)
                        start=(ys,xs)
                        print("No blanco")
