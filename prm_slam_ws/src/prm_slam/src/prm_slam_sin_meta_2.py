...
def generar_puntos_aleatorios_radio(self, imagen_gris, imagen_color):
    ...
    for (px, py) in self.puntos_guardados_pix:
...
def prm(self, image, im_bin, image_color, num_muestras, distancia_max, punto_inicio, punto_destino):
    ...
    while path is None:
        ...
        grafo = nx.Graph()
        muestras_validas = generar_muestras_validas(im_bin, num_muestras)
...
class Nodo(object):
    ...
    def start(self):
        ...
        while not rospy.is_shutdown() and self.contador < self.limite:
            if self.x is not None and self.Xhd is not None:
                ...
                if distancia_pcm < 0.25:
                 ...
                 #Obtener imagen del mapa
                    ...
                    image_color=cv2.cvtColor(imagen_binaria, cv2.COLOR_GRAY2BGR)
                    ...
                    imagen_origen[j,sp] = [255, 150, 0]  # Morado
                    ...
                    self.path=prm(self, img, img_bin, image_color, num_samples, k_neighbors, start, goal)
